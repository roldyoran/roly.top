	...
	<div class="min-h-screen flex items-center justify-center px-4 relative z-10">
		<div class="w-full max-w-md">
			<div class="rounded-2xl border border-border bg-card p-8 text-center space-y-6">
				<!-- Error de autenticación (cancelar login, etc.) -->
				<template v-if="isAuthError">
					...
					<div class="space-y-2">
						...
						<p class="text-sm text-muted-foreground leading-relaxed">
							{{ authErrorMessage || "Ocurrió un problema al iniciar sesión. Inténtalo de nuevo." }}
						</p>
					</div>
...
import { AlertCircle, Clock, Info, ShieldOff } from "lucide-vue-next";
...
// Detectar si es un error de autenticación (cancelar login, error OAuth, etc.)
// Better Auth redirige a /auth/error con query params como ?error=access_denied
const isAuthError = computed(() => {
	// Si hay parámetros de error en la URL, es un error de auth
	if (route.query.error) return true;
	// Si no hay sesión y no está baneado, es un error de auth
	if (!authStore.isAuthenticated && !authStore.isBanned) return true;
	return false;
});

const authErrorMessage = computed(() => {
	const error = route.query.error;
	if (error === "access_denied") return "Has cancelado el inicio de sesión.";
	if (error === "cancelled") return "El inicio de sesión fue cancelado.";
	if (typeof route.query.error_description === "string") {
		return decodeURIComponent(
			route.query.error_description.replace(/\+/g, " "),
		);
	}
	return null;
});
